...
async def async_migrate_entry(hass, config_entry: ConfigEntry):
    logger.debug(
        "[migrate_entry] Nothing to do from version %s to version %s",
...
async def async_setup_entry(hass: HomeAssistant, config: ConfigEntry):
    ...
    latitude = (
        config.data.get(CONF_LATITUDE)
        if config.data.get(CONF_LATITUDE) is not None
        else hass.config.latitude
    )
    longitude = (
        config.data.get(CONF_LONGITUDE)
        if config.data.get(CONF_LONGITUDE) is not None
        else hass.config.longitude
    )
    radius = (
        config.data.get(CONF_RADIUS) if config.data.get(CONF_RADIUS) is not None else 50
    )
    county = (
        config.data.get(CONF_COUNTY)
        if config.data.get(CONF_COUNTY) is not None
        else None
    )
    country = (
        config.data.get(CONF_COUNTRY)
        if config.data.get(CONF_COUNTRY) is not None
        else None
    )
